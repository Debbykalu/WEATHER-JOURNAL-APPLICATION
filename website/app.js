...
function saveWeatherData(data) {
  ...
  displayWeatherData(weatherData).then(function() {
    postWeatherData("lastEntry", weatherData).then(() => {});
  });
}
...
const displayWeatherData = async weatherData => {
  ...
  if (weatherData.date === undefined) {
    msg.innerHTML =
      " Please enter zip code and click on the generate button to get your weather forecast!";
    return;
  }
  msg.innerHTML = "";
  ...
  let tempInfo = `<img src="${iconPath}${
    weatherData.icon
  }@2x.png" alt=""/><p>${Math.floor(weatherData.temp)} &#8457</p>`;
  let tempDiv = document.getElementById("temp");
  tempDiv.innerHTML = tempInfo;

  let description = `<p> ${weatherData.description}, feels like: ${Math.floor(
